...
setTimeout(function() {CachedEvent.emit('glideform.initialized', this)}, 0);
},
...
catch (exception) { alertError("formatClean",exception); }
return sVal;
}
...
if (num.lastIndexOf(".") < 0) { num = num + '00'; }
num = formatClean(num);
...
catch (exception) { alertError("Format Dollar",e); }
return samount;
}
...
Table.get = function(tableName, parentTable, isTemplate) {
var topWindow = getTopWindow(tableName, parentTable, isTemplate);
return !topWindow.Table ? Table.getInTopWindow(tableName, parentTable, isTemplate) : topWindow.Table.getInTopWindow(tableName, parentTable, isTemplate);
}
...
Table.getInTopWindow = function(tableName, parentTable, isTemplate) {
var t = new Object();
Table.setCachable(t);
if (t.cacheable) {
var cachedName = Table.getCacheKey(tableName, parentTable, isTemplate);
...
var answer = new Table(tableName, parentTable, null, null, null, isTemplate);
if (parentCache && answer.cacheable)
parentCache.put(cachedName, answer);
return answer;
}
...
;
/*! RESOURCE: /scripts/classes/ISO9075.js */
var ISO9075 = Class.create({
ENCODE_PATTERN: /_x[A-Fa-f0-9]{4}_/gi,
CHARS: [],
HEX_DIGITS: "0123456789abcdefABCDEF",
MASK_VALID: 0x01,
MASK_SPACE: 0x02,
MASK_NAME_START: 0x04,
MASK_NAME: 0x08,
MASK_PUBID: 0x10,
MASK_CONTENT: 0x20,
MASK_NCNAME_START: 0x40,
MASK_NCNAME: 0x80,
initialize: function () {
this.initChars();
},
...
for (var i = 0; i < charRange.length; i += 2) {
for (var j = charRange[i]; j <= charRange[i + 1]; j++)
CHARS[j] |= this.MASK_VALID | this.MASK_CONTENT;
}
for (var i = 0; i < specialChar.length; i++) {
CHARS[specialChar[i]] = (CHARS[specialChar[i]] & ~this.MASK_CONTENT);
}
for (var i = 0; i < spaceChar.length; i++) {
CHARS[spaceChar[i]] |= this.MASK_SPACE;
}
for (var i = 0; i < nameStartChar.length; i++) {
CHARS[nameStartChar[i]] |= this.MASK_NAME_START | this.MASK_NAME |
this.MASK_NCNAME_START | this.MASK_NCNAME;
}
for (var i = 0; i < letterRange.length; i += 2) {
for (var j = letterRange[i]; j <= letterRange[i + 1]; j++)
CHARS[j] |= this.MASK_NAME_START | this.MASK_NAME | this.MASK_NCNAME_START | this.MASK_NCNAME;
}
for (var i = 0; i < letterChar.length; i++) {
CHARS[letterChar[i]] |= this.MASK_NAME_START | this.MASK_NAME | this.MASK_NCNAME_START | this.MASK_NCNAME;
}
for (var i = 0; i < nameChar.length; i++) {
CHARS[nameChar[i]] |= this.MASK_NAME | this.MASK_NCNAME;
}
for (var i = 0; i < digitRange.length; i += 2) {
for (var j = digitRange[i]; j <= digitRange[i + 1]; j++)
CHARS[j] |= this.MASK_NAME | this.MASK_NCNAME;
}
for (var i = 0; i < combiningCharRange.length; i += 2) {
for (var j = combiningCharRange[i]; j <= combiningCharRange[i + 1]; j++)
CHARS[j] |= this.MASK_NAME | this.MASK_NCNAME;
}
for (var i = 0; i < combiningCharChar.length; i++) {
CHARS[combiningCharChar[i]] |= this.MASK_NAME | this.MASK_NCNAME;
}
for (var i = 0; i < extenderRange.length; i += 2) {
for (var j = extenderRange[i]; j <= extenderRange[i + 1]; j++)
CHARS[j] |= this.MASK_NAME | this.MASK_NCNAME;
}
for (var i = 0; i < extenderChar.length; i++) {
CHARS[extenderChar[i]] |= this.MASK_NAME | this.MASK_NCNAME;
}
CHARS[':'.charCodeAt(0)] &= ~(this.MASK_NCNAME_START | this.MASK_NCNAME);
for (var i = 0; i < pubidChar.length; i++) {
CHARS[pubidChar[i]] |= this.MASK_PUBID;
}
for (var i = 0; i < pubidRange.length; i += 2) {
for (var j = pubidRange[i]; j <= pubidRange[i + 1]; j++)
CHARS[j] |= this.MASK_PUBID;
}
this.CHARS = CHARS;
},
encodeChar: function (c, buf) {
buf.push("_x");
...
needsEscaping: function (name, location) {
...
if (name.length == 0)
return name;
if (this.isValidName(name) && name.indexOf("_x") < 0) {
return name;
}
var buf = [];
for (var i = 0; i < name.length; i++) {
if (i == 0) {
if (this.isNameStart(name.charCodeAt(0))) {
if (this.needsEscaping(name, i)) {
this.encodeChar("_", buf);
} else
buf.push(name.charAt(i));
} else {
this.encodeChar(name.charAt(i), buf);
}
} else if (!this.isName(name.charCodeAt(i)))
this.encodeChar(name.charAt(i), buf);
else {
if (this.needsEscaping(name, i)) {
this.encodeChar('_', buf);
...
return name;
var str2 = name.replace(this.ENCODE_PATTERN, function (match) {
return String.fromCharCode("0" + match.toString().substring(1, 6))
});
return str2;
},
...
if (this.isNameStart(ch.charCodeAt(0)) == false)
return false;
for (var i = 1; i < name.length; i++) {
ch = name.charAt(i);
if (this.isName(ch.charCodeAt(0)) == false)
return false;
}
return true;
},
isNameStart: function (c) {
return c < 0x10000 && (this.CHARS[c] & this.MASK_NAME_START) != 0;
},
isName: function (c) {
return c < 0x10000 && (this.CHARS[c] & this.MASK_NAME) != 0;
},
type: "ISO9075"
});
;
...
if (!value)
value = "";
fields.push({ 'name' : iso.decode(name), 'value' : value });
}
...
continue;
var f = xml.createElement(iso.encode(xname));
item.appendChild(f);
var v = this[xname];
if (!v)
v = "NULL";
